...
#    @context = @submission.context
    ...
    @file_records = (nil and @submission_revision) ? %w[author_file author_expert_file].map do |type|
...
#      end
  ...
  private
    ...
    def set_breadcrumbs
      ...
      add_breadcrumb "Author", context_submissions_path(@context), dropdown: {
        list: [
          ...
          {name: :ce, text: 'Chief Editor', path: '/chief_editor'},
    ...
    def submission_params
      params.require(:submission).permit(:title, :abstract, :sid, :user_id, :context_id, :revision_seq, :last_created_revision_id, :last_submitted_revision_id, :aasm_state)
    end
end
